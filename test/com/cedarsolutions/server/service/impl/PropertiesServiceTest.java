...
/**
 * Unit test for PropertiesService, identical to the tests for PropertiesUtils.
 * @author Kenneth J. Pronovici <pronovic@ieee.org>
 */
public class PropertiesServiceTest {
    ...
    @Test public void testLoadPropertiesValid() throws Exception {
        try {
            ...
            try {
                getService().loadProperties("build/tmp/PropertiesUtilsTest/valid.properties");
            ...
            Properties properties = getService().loadProperties("build/tmp/PropertiesUtilsTest/valid.properties");
    ...
    @Test public void testLoadPropertiesInvalid() throws Exception {
        try {
            ...
            try {
                getService().loadProperties("build/tmp/PropertiesUtilsTest/invalid.properties");
                fail("Expected InvalidDataException");
            } catch (InvalidDataException e) { }
        } finally {
            FilesystemUtils.removeFile("build/tmp/PropertiesUtilsTest/invalid.properties");
    ...
    public void testGetMatchingPrefixesNull() {
        ...
        List<String> matches = getService().getMatchingPrefixes(properties, "prefix");
    ...
    public void testGetMatchingPrefixesNonParent() {
        Properties properties = createProperties();
        List<String> matches = getService().getMatchingPrefixes(properties, "b");
        assertTrue(matches.isEmpty());
    }
    ...
    public void testGetMatchingPrefixesSingleParent() {
        Properties properties = createProperties();
        List<String> matches = getService().getMatchingPrefixes(properties, "c");
    ...
    public void testGetMatchingPrefixesMultipleParents() {
        Properties properties = createProperties();
        List<String> matches = getService().getMatchingPrefixes(properties, "a");
        assertFalse(matches.isEmpty());
    ...
    public void testGetMatchingPrefixesVarious() {
        Properties properties = createProperties();
        List<String> matches = getService().getMatchingPrefixes(properties, "starts.with.d");
        assertFalse(matches.isEmpty());
        assertEquals(2, matches.size());
    ...
    public void testGetMatchingChildrenNull() {
        Properties properties = null;
        List<String> matches = getService().getMatchingChildren(properties, "prefix");
        assertTrue(matches.isEmpty());
    }
    ...
    public void testGetMatchingChildrenNonParent() {
        Properties properties = createProperties();
        List<String> matches = getService().getMatchingChildren(properties, "b");
        assertFalse(matches.isEmpty());
        assertEquals(1, matches.size());
    ...
    public void testGetMatchingChildrenSingleParent() {
        Properties properties = createProperties();
        List<String> matches = getService().getMatchingChildren(properties, "c");
        assertFalse(matches.isEmpty());
        assertEquals(1, matches.size());
        assertEquals("c.one", matches.get(0));
    }
    ...
    public void testGetMatchingChildrenMultipleParents() {
        Properties properties = createProperties();
        List<String> matches = getService().getMatchingChildren(properties, "a");
        assertFalse(matches.isEmpty());
        assertEquals(2, matches.size());
        assertTrue(matches.contains("a.two"));
        assertTrue(matches.contains("a.three"));
    ...
    public void testGetMatchingChildrenVarious() {
        Properties properties = createProperties();
        List<String> matches = getService().getMatchingChildren(properties, "starts.with.d");
        assertFalse(matches.isEmpty());
    ...
    private static IPropertiesService getService() {
