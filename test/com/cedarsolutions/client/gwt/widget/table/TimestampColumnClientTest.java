...
/**
 * Client-side unit tests for TimestampColumn.
 * @author Kenneth J. Pronovici <pronovic@ieee.org>
 */
public class TimestampColumnClientTest extends ClientTestCase {

    /** Test constructor and getName() method. */
    public void testConstructor() {
        TimestampColumn<Row> column  = new TestColumn();
    ...
    public void testFormatField() {
        ...
        assertEquals("2013-11-15T18:32:13,026", column.formatField(field));

        column  = new TestColumn();
        column.setIncludeZone(true);
        row.setValue(GwtDateUtils.createDate(2013, 11, 15, 18, 32, 13, 26));
        field = column.getField(row);
        assertTrue(column.formatField(field).startsWith("2013-11-15T18:32:13,026"));
    }

    /** Test column to work with. */
    private static class TestColumn extends TimestampColumn<Row> {
        public TestColumn() {
            super();
        }
