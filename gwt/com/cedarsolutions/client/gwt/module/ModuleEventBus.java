/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 *
 *              C E D A R
 *          S O L U T I O N S       "Software done right."
 *           S O F T W A R E
 *
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 *
 * Copyright (c) 2011-2012 Kenneth J. Pronovici.
 * All rights reserved.
 *
 * This program is free software; you can redistribute it and/or
 * modify it under the terms of the Apache License, Version 2.0.
 * See LICENSE for more information about the licensing terms.
 *
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 *
 * Author   : Kenneth J. Pronovici <pronovic@ieee.org>
 * Language : Java 6
 * Project  : Common Java Functionality
 *
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
package com.cedarsolutions.client.gwt.module;

import com.google.gwt.user.client.ui.IsWidget;

/**
 * Event bus for a module.
 * @author Kenneth J. Pronovici <pronovic@ieee.org>
 */
public interface ModuleEventBus extends SessionAwareEventBus {

    /** Set the contents of the root body, replacing the previous contents. */
    void replaceRootBody(IsWidget contents);

    /** Set the contents of the module template body, replacing the previous contents. */
    void replaceModuleBody(IsWidget contents);

}
